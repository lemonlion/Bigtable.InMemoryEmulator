...
Every piece of behavioral logic in the source code — status codes, validation rules, error conditions, side-effect semantics — **must** be backed by a verified source. This prevents accidental divergence from real Google Cloud Bigtable behavior.
...
2. **Add a code comment** at the implementation site citing the source (a short URL or description is sufficient). Example:
   ...
   //   "Return rows in lexiographical descending order of the row keys."
   ```
...
|----------|--------|----------------|
| 1 | Google Cloud Bigtable gRPC API reference | https://cloud.google.com/bigtable/docs/reference/data/rpc/google.bigtable.v2 |
| 2 | Google Cloud Bigtable Admin API reference | https://cloud.google.com/bigtable/docs/reference/admin/rpc/google.bigtable.admin.v2 |
| 3 | Google Cloud Bigtable .NET SDK API reference | https://cloud.google.com/dotnet/docs/reference/Google.Cloud.Bigtable.V2/latest |
| 4 | Google Cloud Bigtable "How-to" guides | https://cloud.google.com/bigtable/docs/how-to |
| 5 | Google Cloud Bigtable conceptual docs | https://cloud.google.com/bigtable/docs/concepts |
| 6 | Google Cloud Bigtable .NET SDK source code | https://github.com/googleapis/google-cloud-dotnet |
| 7 | Observed behavior on the Go Bigtable Emulator | (local testing via `gcloud beta emulators bigtable start`) |

> **Note:** Source 7 (Go emulator observation) is the weakest evidence. The Go emulator has known gaps (no GoogleSQL, no ReadChangeStream, no Sink filter). Always cross-reference with sources 1–6 when possible.
...
### Tests.Integration
- Uses `TestFixtureFactory.Create(session)` / `ITestTableFixture` to obtain a table, where `session` is an injected `EmulatorSession` (xUnit collection fixture — decorate the test class with `[Collection(IntegrationCollection.Name)]`)
- Goes through the real BigtableClient SDK gRPC pipeline via the in-process gRPC TestServer
- Must **not** use `new InMemoryBigtableStore()`, `new FaultInjector()`, or any `internal` API
- Can run against in-memory, Go emulator, or real GCP Bigtable via `BIGTABLE_TEST_TARGET`
- **This is the primary test project** — every test should be an integration test unless it requires internal API access

### Tests.Unit
- Uses `new InMemoryBigtableStore()`, `new InMemoryBigtableClient()`, or any `internal` API directly
